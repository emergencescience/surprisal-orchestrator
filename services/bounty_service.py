...
from services.execution import execute_submission_sync, validate_code_safety
...
class BountyService:
    ...
    def create_bounty(session: Session, bounty_in: BountyCreate, owner: User) -> Bounty:
        ...
        existing_bounty = session.exec(
            select(Bounty).where(
                Bounty.owner_id == owner.id,
                Bounty.idempotency_key == bounty_in.idempotency_key
            )
        ).first()
        if existing_bounty:
        ...
        if reward_micro > CreditService.SCALE: # 1,000,000 micro-credits = 1 Credit ($1)
            raise HTTPException(
                status_code=400,
                detail=f"Reward limit exceeded for v1.0.0. Maximum reward is 1.0 Credit ({CreditService.SCALE} micro-credits)."
            )
        ...
        if language != "python3":
            raise HTTPException(
                status_code=400, 
                detail=f"Language '{language}' is not supported during the protocol incubation phase. Please use 'python3'."
            )
        ...
        try:
            ...
            # Mandatory Dry-Run
            # We use the solution_template as the dry-run solution if provided, else use an empty string/pass
            ...
            # Note: We don't REQUIRE the dry-run to pass (status="accepted"), 
            # but we REQUIRE it to successfully EXECUTE without system errors.
        ...
        if owner.micro_credits < total_cost:
            raise HTTPException(
                status_code=402,
                detail=f"Insufficient credits. Total cost (Reward + Listing Fee) is {CreditService.from_micro(total_cost)} credits."
            )
        ...
        if CreditService.LISTING_FEE > 0:
            fee_txn = Transaction(
                ...
                amount=CreditService.LISTING_FEE,
                bounty_id=bounty.id,
                type="fee"
            )
            session.add(fee_txn)
    ...
    def create_submission(session: Session, bounty_id: uuid.UUID, solver: User, submission_in: SubmissionCreate) -> Submission:
        ...
        existing_submission = session.exec(
            select(Submission).where(
                ...
                Submission.idempotency_key == submission_in.idempotency_key
            )
        ).first()
        ...
        try:
            ...
            session.rollback()
            # Try to find existing submission for idempotency
            ...
            if existing:
                return existing
            # If not idempotency, it's a real integrity error (e.g. NOT NULL)
    ...
    def delete_bounty(session: Session, bounty_id: uuid.UUID, user: User):
        ...
        if bounty.status != "deleted" and bounty.reward > 0:
