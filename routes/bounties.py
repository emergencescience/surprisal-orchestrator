...
def create_bounty(
    ...
    return BountyRead(
        **bounty_obj.model_dump(exclude={"reward"}),
        reward=CreditService.from_micro(bounty_obj.reward),
        micro_reward=bounty_obj.reward
    )
...
def get_bounties(
    ...
    bounties = session.exec(
        query.order_by(Bounty.created_at.desc()).offset(skip).limit(limit)
    ).all()
    
    return [
        BountyRead(
            **b.model_dump(exclude={"reward"}),
            reward=CreditService.from_micro(b.reward),
            micro_reward=b.reward
        ) for b in bounties
    ]
...
def get_bounty(bounty_id: uuid.UUID, session: Session = Depends(get_session)):
    ...
    return BountyDetailRead(
        **bounty.model_dump(exclude={"reward"}),
        reward=CreditService.from_micro(bounty.reward),
        micro_reward=bounty.reward
    )

@router.get("/{bounty_id}/solution")
def get_bounty_solution(
    bounty_id: uuid.UUID, 
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
