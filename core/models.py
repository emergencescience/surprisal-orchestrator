...
class User(UserBase, table=True):
    __table_args__ = (
        sa.UniqueConstraint("provider", "provider_id", name="unique_user_provider"),
    ...
    micro_credits: int = Field(
        ...
        sa_column=sa.Column(
            sa.BigInteger, 
            default=lambda: int(os.getenv("INITIAL_GRANT_MICRO_CREDITS", "1000000"))
        )
    )
...
class TransactionRead(SQLModel):
    ...
    amount: float # External Credits (1.0 = $1)
...
class Bounty(BountyBase, table=True):
    ...
    accepted_submission_id: uuid.UUID | None = Field(
        ...
        sa_column=sa.Column(
            sa.UUID, 
            sa.ForeignKey("submission.id", use_alter=True, name="fk_bounty_accepted_submission")
        )
    )
...
class Submission(SubmissionBase, table=True):
    __table_args__ = (
        sa.UniqueConstraint("solver_id", "idempotency_key", name="unique_submission_idempotency"),
        {"extend_existing": True}
    )
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    bounty_id: uuid.UUID = Field(
        sa_column=sa.Column(
            sa.UUID, 
            sa.ForeignKey("bounty.id", name="fk_submission_bounty"), 
            nullable=False,
            index=True
        )
    )
